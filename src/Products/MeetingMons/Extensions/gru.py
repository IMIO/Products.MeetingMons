...
class TransformXmlToMeetingOrItem:
    ...
    def addMeetingAnnexe(self, meeting, node):
        ...
        """
        #nous utiliserons le répertoire de GRUImport
        Memberfolder = self.__portal__.Members.gruimport.mymeetings.get('meeting-config-college')
        text = 'file:\\\\D:\\GRU\\Software\\Tomcat\\webapps\\isf_beGov\\pdf-files\\'
        annexe = self.getText(node).replace(text,
                                            '/home/zope/pdf-files/')
    ...
    def addItemPDFPoint(self, item, node, Memberfolder):
        text = 'file:\\\\D:\\GRU\\Software\\Tomcat\\webapps\\isf_beGov\\pdf-files\\'
        _path = self.getText(node).replace(text,
                                           '/home/zope/pdf-files/')
    ...
    def addItemAnnexes(self, item, node, Memberfolder):
        ...
        for annexes in node.getElementsByTagName("annexLink"):
            try:
                text = "file:\\\\D:\\GRU\\Data\\Annexes\\"
                _path = self.getText(node.getElementsByTagName(
                    "annexLink")[i]).replace(text, "/home/zope/Annexes")
                title = 'Annexe-%d' % i
    ...
    def addItemAdvises(self, item, node, Memberfolder):
        ...
        for annexes in node.getElementsByTagName("adviseLink"):
            try:
                text = 'file:\\\\D:\\GRU\\Software\\Tomcat\\webapps\\isf_beGov\\pdf-files\\'
                _path = self.getText(node.getElementsByTagName("adviseLink")[i]).replace(
                    text, '/home/zope/pdf-files/')
                title = 'Avis-%d' % i
        ...
        for meetings in self.getRootElement().getElementsByTagName("seance"):
            if meetings.nodeType == meetings.ELEMENT_NODE:
                try:
                    ...
                    #ajout d'une observation comprenant un lien vers le pdfSeanceLink
                    # (pour Marche il y en a toujours qu'un)
                    try:
